    ...
    for example_index, example in enumerate(examples):
        ...
        for ending_index, ending in enumerate(example.endings):
            ...
            choices_features.append(
                (tokens, input_ids, input_mask, segment_ids, vp_tokens, vp_input_ids, vp_input_mask))
        ...
        if example_index < 5:
            ...
            for choice_idx, (
            tokens, input_ids, input_mask, segment_ids, vp_tokens, vp_input_ids, vp_input_mask) in enumerate(
                    choices_features):
...
def main():
    ...
    if args.do_train:
        ...
        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label, all_vp_input_ids,
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                golden_vps = label_ids.view(-1, 1).repeat(1, vp_input_ids.size(2)).view([-1, 1, vp_input_ids.size(2)])
                vp_input_ids = torch.gather(vp_input_ids, dim=1, index=golden_vps)
                vp_input_ids = vp_input_ids.view([vp_input_ids.size(0), -1])
                vp_input_mask = torch.gather(vp_input_mask, dim=1, index=golden_vps)
                vp_input_mask = vp_input_mask.view([vp_input_mask.size(0), -1])
                vp_input_ids = torch.cat([100 * torch.ones(vp_input_ids.shape[0], 1).long().to(device), vp_input_ids], dim=1)
                vp_input_mask = torch.cat([torch.ones(vp_input_ids.shape[0], 1).long().to(device), vp_input_mask], dim=1)
...
#                    protected_attr_ids.shape[0] * protected_attr_ids.shape[1])
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label, all_vp_input_ids,
                                  all_vp_input_mask, all_protected_attr)
